...
const (
	...
	SetUpSelectForeignInternal = "SELECT table_name, column_name, referenced_table_schema, referenced_table_name, referenced_column_name FROM information_schema.key_column_usage WHERE table_schema = '%s' and table_name IN (%s) and referenced_table_name is not null;"
	SetUpSelectForeignExternal = "SELECT referrer_table, referrer_field, referenced_schema, referenced_table, referenced_field FROM ref_init WHERE referrer_schema = '%s' AND referrer_table in (%s);"
