...
DROP DATABASE IF EXISTS `vooo_prod_listener`;
CREATE DATABASE `vooo_prod_listener`;
USE `vooo_prod_listener`;
...
CREATE TABLE `client_job_scheduler` (
  ...
  # ajuste Paulo
  # CONSTRAINT `client_job_scheduler_ibfk_1` FOREIGN KEY (`client_id`, `connector_id`) REFERENCES `vooo_prod_backend`.`client_connector` (`client_id`, `connector_id`)
